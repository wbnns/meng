...
def generate_report(result, current_date):
   ...
   token_data = list(result["token_security"]["result"].values())[0]
...
def save_report(report, token_symbol, current_date):
   """
   Save the report to a Markdown file named after the token symbol and current date.
   ...
   filename = f"reports/{token_symbol}_{current_date}.md"
...
def main():
   ...
   if result:
       ...
       token_symbol = list(result["token_security"]["result"].values())[0].get("token_symbol", "unknown")
